...
def render_only_image(camera, renderer, vertices, color, focal_length, res, img) :
...
def visualize_reconstruction(img, img_size, gt_kp, vertices, pred_kp, camera, renderer, color='pink', focal_length=1000, trainMode = True):
    ...
    rend_img = render_only_image(camera, renderer, vertices, color, focal_length, res, img)
    try :
        if(trainMode) :
    ...
    image_with_ground_truth = draw_skeleton( img, gt_joint, draw_edges=False, vis=gt_vis)

    skel_image = draw_skeleton(image_with_ground_truth, pred_joint)

    combined = np.hstack([skel_image, rend_img])
