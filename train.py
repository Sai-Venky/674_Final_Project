...
import numpy as np
import cv2
from src.models import BertConfig, Graphormer
from src.models import Graphormer_Hand_Network as Graphormer_Network
from src.models.mano import MANO, Mesh
from src.models.hrnet.hrnet_cls_net_gridfeat import get_cls_net_gridfeat
from src.models.hrnet.config import config as hrnet_config
from src.models.hrnet.config import update_config as hrnet_update_config
import src.utils.config as cfg
from src.utils.build import make_hand_data_loader
...
def run(args, train_dataloader, Graphormer_model, mano_model, renderer, mesh_sampler):
    ...
    max_iter = 1000 # len(train_dataloader)
    ...
    for iteration, (img_keys, images, annotations) in enumerate(train_dataloader):
        ...
        if iteration % 10 == 1 : # args.logging_steps == 0 or iteration == max_iter:
            ...
            print("Losses")
            print(float(log_losses.avg))
            print(float(log_loss_3djoints.avg))
...
def parse_args():
    ...
    parser.add_argument("--train_yaml", default='./datasets/freihand/train.yaml', type=str, required=False,
                        help="Yaml file with all data for training.") # imagenet2012
    ...
    parser.add_argument("--num_hidden_layers", default=2, type=int, required=False,
                        help="Update model config if given")  # 4
